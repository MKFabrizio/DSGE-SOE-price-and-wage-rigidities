...
if ~options_.ramsey_policy
    outvalue=y_sim(strmatch('welfgap',M_.endo_names,'exact'),2)*100; %read out gap
else
    welfeq=y_sim(strmatch('welfeq',M_.endo_names,'exact'),2);
    oo_.steady_state=oo_.dr.ys;
    oo_.planner_objective_value = evaluate_planner_objective(M_,options_,oo_);
    outvalue=(oo_.planner_objective_value(1)-welfeq)*100;
end