...
set_param_value('PPHI_C',par_value_lambda) %set consumption equivalent lambda
if ~options_.ramsey_policy
    ...
    outvalue=oo_.mean(strmatch('welfgap',var_list_,'exact')); %extract Welfare gap measure
else
    ...
    %for welfare
    var_list_ = char('util','welfeq');
    info = stoch_simul(var_list_);
    if info(1)
        outvalue=1e5+par_value_lambda^2;
        return;
    end
    BBETA=M_.params(strmatch('BBETA',M_.param_names,'exact'));
    outvalue=1/(1-BBETA)*oo_.mean(strmatch('util',var_list_ ,'exact'))-oo_.mean(strmatch('welfeq',var_list_,'exact'));
end